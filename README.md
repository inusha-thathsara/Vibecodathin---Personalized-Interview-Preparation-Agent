...
- **Deterministic Phase Controller**: Manages state transitions across `INTRO`, `CORE`, `FOLLOW_UP`, and `WRAP_UP` phases, guaranteeing at least 8 primary questions across 4+ curriculum days before structured conclusion.
...
- **Environment-Aware LLM Routing**:
  - **Local Development**: Runs with **Ollama** (`OLLAMA_MODEL=gemma4:latest` or `qwen2.5`) with no Gemini key required.
  - **Production (Render)**: Connects directly to **Gemini API** (`gemini-2.0-flash`).
  - **Automated Testing**: Offline mock provider (`APP_ENV=test`).
- **Rich Structured Feedback & Export**: Produces executive summary, strengths, knowledge gaps, next steps, and curriculum topic scores with one-click **Markdown Export** (Copy & Download).
- **SQLite Session Persistence**: Maintains session history across restarts with automated 2-hour idle TTL cleanup.
- **Real-Time Streaming**: Supports Server-Sent Events (SSE) via `POST /api/interview/stream`.
...
### 1. Prerequisites
- Python 3.10+
- (Optional) [Ollama](https://ollama.com/) running locally for development.
...
The test suite covers candidate analysis, engagement scoring, heuristic nonsense detection, phase state transitions, RAG retrieval, feedback schema generation, and FastAPI endpoints:
...
- `GET /health`: Health status and provider health check.
- `GET /api/candidates`: List all graduating candidates and history profiles.
- `POST /api/interview`: Standard API spec endpoint (`reply`, `done`, `feedback`, `meta`).
- `POST /api/interview/stream`: Server-Sent Events (SSE) streaming endpoint.

---

## 🧩 Live Steer & Extension Points
...
- **RAG & Curriculum Indexing**: [`rag/retriever.py`](file:///e:/Documents/Projects/VibeCodathon/rag/retriever.py)
